...
const RECONNECT_MAX_DELAY_MS = 5 * 60 * 1000; // 5 minutes cap
...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    // Incremented on every connectSocket() call. Stale close events from
    // abandoned sockets (never explicitly ended) check this before mutating
    // shared state — prevents a lagging old socket from setting connected=false
    // after the new socket has already opened.
    let socketGeneration = 0;
    // Cache the WA Web version for the lifetime of this process so reconnects
    // don't depend on an external fetch to wppconnect.io every time.
    ...
    function scheduleReconnect(delayMs: number): void {
      ...
      setTimeout(() => {
        ...
        connectSocket().catch((err) => {
          const next = Math.min(delayMs * 2, RECONNECT_MAX_DELAY_MS);
          log.error('Reconnect attempt failed, will retry', { err, nextDelayMs: next });
    ...
    async function connectSocket(): Promise<void> {
      ...
      if (!cachedWaVersion) {
        cachedWaVersion = await resolveWaWebVersion();
      }
      ...
      sock.ev.on('connection.update', (update) => {
        // Ignore events from sockets superseded by a later connectSocket() call.
        ...
        if (connection === 'close') {
          ...
          if (shouldReconnect) {
            scheduleReconnect(RECONNECT_INITIAL_DELAY_MS);
